...
trailblazer_scoring <- trailblazer_longer |>
  ...
  mutate(mean_home = mean(Home, na.rm = TRUE)) |> #creating a column for mean home points
  mutate(mean_away = mean(Away, na.rm = TRUE)) |> #creating a column for mean away points
  mutate(mean_diff = mean_home - mean_away) |> #creating a column for the difference in home vs away points
...
```{r}
penguins |>
  mutate(bill_length_mm = case_when(is.na(bill_length_mm) & species == "Adelie" ~ 26,
